...
// Here, Triangle’s constructor initializes the members of TwoDShape that it inherits along
// with its own Style field.

// When both the base class and the derived class define constructors, the process is a
// bit more complicated because both the base class and derived class constructors must be
// executed. In this case, you must use another of C#’s keywords, base, which has two uses.
// The first use is to call a base class constructor. The second is to access a member of the base
// lass that has been hidden by a member of a derived class. Here, we will look at its first use.
